...
(function() {
    ...
    jigsaw.Jigsaw = Class.extend({
        init: function(opts) {
            ...
            this.ui = new jigsaw.UI(eventBus, 9);
        ...
        shuffle: function() {
            ...
            var i, l = T.length,
				...
				var woffset=(width/2)-((T[0].width)+T[0].width+10);	
if(pixelRatio>1){
woffset=(width/2)-((T[0].width)+T[0].width+10);
}
if(window.devicePixelRatio >1){
			   ...
			   calcY2=calcY2-50;
				...
				var wff=woffset;
				var xx=[woffset,woffset+(T[0].width*3)+30,woffset+(T[0].width*4)+40,woffset+(T[0].width)+10,woffset+(T[0].width*2)+20];
				var xxx=[wff,wff+(T[0].width)+10,wff+(T[0].width*2)+20,wff+(T[0].width*3)+30];
				var rand_index2=Math.floor(Math.random()*4),counter=0;
